...
type RejectPaymentReceiptMutationInput = {
  receiptId: string;
  input: PaymentReceiptReviewActionInput;
};

export function useRejectPaymentReceipt(orderId: string) {
  const queryClient = useQueryClient();

  return useMutation<OrderDetail, FetcherError, RejectPaymentReceiptMutationInput>({
    mutationFn: ({ receiptId, input }) => crmApiClient.rejectPaymentReceipt(orderId, receiptId, input),

...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    ensureNotSelfMutation(adminUser.id, params.userId, "desbloquear");

    if (!target.email) {
      throw badRequest("El usuario no tiene correo para enviar reset.");
    }

    await unlockUserByAdmin(service, {
      adminUserId: adminUser.id,
    ...
    return ok({ unlocked: true, resetSent: true });
  } catch (error) {
